...
def define_actors(find_and_replace):
    def headers_lambda(a):
        copy = a.copy()
        copy['ACTORS'] = 'anything - only for header'
        return copy.keys()

    def update(a):
        parties = a['SPONSOR_PARTIES']
        if "L'Ulivo" in parties and "Forza Italia" in parties:
            a["ACTORS"] = "cross-party"
        elif "L'Ulivo" in parties:
            a["ACTORS"] = "gov_bb"
        elif "Forza Italia" in parties:
            a["ACTORS"] = "opposition"
        elif "Lega Nord" in parties:
            a["ACTORS"] = "opposition"
        else:
            a["ACTORS"] = ""
        return a
...
if __name__ == '__main__':
    ...
    define_actors(FindAndReplace('./outputs/15.final_parties.csv', './outputs/15.actors_coded.csv'))
